...
import fragments.MyAvailabilityFragment;
import fragments.MyUnavailabilityFragment;
import models.Slot;
...
public class UpdateAvailabilityActivity extends Legion_BaseActivity implements View.OnClickListener, Legion_NetworkCallback, Legion_Constants {
    ...
    private MyUnavailabilityFragment unavailabilityFragment;
    private MyAvailabilityFragment availabilityFragment;
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.whenIPreferToWork:
                ...
                availabilityFragment.doUpdateSlots();
                break;
            ...
            case R.id.whenIamBusy:
                ...
                unavailabilityFragment.doUpdateSlots();
                break;

            case R.id.saveAvailability:
                if (saveAvailability.getCurrentTextColor() == Color.WHITE) {
                    if (!LegionUtils.isOnline(this)) {
                        LegionUtils.showDialog(this, getString(R.string.device_offline_message), false);
                        return;
                    }
            ...
            discardChangesButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (doFinishActivity) {
                        ...
                        saveAvailability.setTextColor(ActivityCompat.getColor(UpdateAvailabilityActivity.this, R.color.swap_text_color));
                        if(isPrev){
                            onPrevWeekClick();
                        }else{
                            onNextWeekClick();
...
/* reqObject.put("firstName", firstName);
    ...
    public void onSuccess(int requestCode, final Object response, final Object headers) {
        if (requestCode == WebServiceRequestCodes.QUERY_AVAILABILITY_PREFRENCE_CODE) {
            new AsyncTask<Void, Void, Void>() {
                ...
                protected Void doInBackground(Void... params) {
                    try {
                        ...
                        if (jsonObject.getString("responseStatus").equals("SUCCESS")) {
                            ...
                            availabilityFragment = new MyAvailabilityFragment(saveAvailability, isScheduleLocked, isPastDate(AVLBTYENDDATE));
                ...
                protected void onPostExecute(Void aVoid) {
                    ...
                    new AsyncTask<Void, Void, Void>() {
                        @Override
                        protected Void doInBackground(Void... params) {
                            unavailabilityFragment = new MyUnavailabilityFragment(saveAvailability, isScheduleLocked, isPastDate(AVLBTYENDDATE));
                            getSupportFragmentManager().beginTransaction().replace(R.id.container2, unavailabilityFragment).commit();
                            return null;
                        }
                    }.execute();
                }
            ...
            try {
                ...
                e.printStackTrace();
                Toast.makeText(UpdateAvailabilityActivity.this, "Something went wrong.", Toast.LENGTH_SHORT).show();
            }
        }
    }
        ...
        try {
            ...
            if (slotsJsonArray != null && slotsJsonArray.length() >= 1) {
                ...
                for (int i = 0; i < size; ++i) {
                    ...
                    if (dayOfWeek == 1) {
                        doUpdateSlotsWithServerData(sundaySlotsList, startMins, endMins, positiveFlag);
                    } else if (dayOfWeek == 2) {
                        doUpdateSlotsWithServerData(mondaySlotsList, startMins, endMins, positiveFlag);
                    } else if (dayOfWeek == 3) {
                        doUpdateSlotsWithServerData(tuesdaySlotsList, startMins, endMins, positiveFlag);
                    } else if (dayOfWeek == 4) {
                        doUpdateSlotsWithServerData(wednesdaySlotsList, startMins, endMins, positiveFlag);
                    } else if (dayOfWeek == 5) {
                        doUpdateSlotsWithServerData(thursdaySlotsList, startMins, endMins, positiveFlag);
                    } else if (dayOfWeek == 6) {
                        doUpdateSlotsWithServerData(fridaySlotsList, startMins, endMins, positiveFlag);
                    } else if (dayOfWeek == 7) {
                        doUpdateSlotsWithServerData(saturdaySlotsList, startMins, endMins, positiveFlag);
                    }
                }
            }
        } catch (Exception e) {
    ...
    private void doUpdateSlotsWithServerData(ArrayList<Slot> slotsList, int startMins, int endMins, boolean positiveFlag) {
        int size = slotsList.size();
        for (int j = 0; j < size; ++j) {
            Slot slot = slotsList.get(j);
...
/*availabilityJsonObj.put("firstName", prefsManager.get(Prefs_Keys.FIRST_NAME));
    ...
    protected void onResume() {
        super.onResume();
        if (container1 != null) {
            if (isOnResumeCalled && container1.getVisibility() == View.VISIBLE) {
                buttonWhenIamBusy.performClick();
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
